...
def mercado_libre(nombre_producto):
    ...
    r = requests.get("https://listado.mercadolibre.com.co/{}#D[A:{}]".format(articulo.replace(' ', '-'), articulo.replace(' ', '-')))
    contenido = r.content
    ...
    for page in range(0, last_page_modified):
        result = requests.get('https://listado.mercadolibre.com.co/{}_Desde_{}_NoIndex_True'.format(articulo.replace(' ', '-'), (page*50)+1))
        ...
        for item in alldivs:
            ...
            precio_antes_tag = item.find('span', {'class': 'andes-money-amount__fraction'})
            if precio_antes_tag:
                data['precio_antes'] = precio_antes_tag.text.strip()  # Extrae y limpia el texto del precio anterior
            else:
                data['precio_antes'] = None  # Si no se encuentra, asigna None

            data['link'] = item.find('a', {'class': ''})['href']
            product_urls.append(data['link'])  # Agregar el enlace del producto a la lista
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    imagen_tag = product_soup.find('img', {'class': 'ui-pdp-image ui-pdp-gallery__figure__image'})
